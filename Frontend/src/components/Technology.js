...
class Technology extends Component 
{
    ...
    callGuardian()
    {
        fetch("https://csci571-node.azurewebsites.net/guardianTechnology").then(res=> { 
        return res.json();
            }).then(news_articles=> this.setState({articles:news_articles}));
    }
    callNyt()
    {
        fetch("https://csci571-node.azurewebsites.net/nytTechnology").then(res=> {
            return res.json();
        ...
        if(localStorage.getItem("checked")==='true')
        {
            ...
            return ( 
                <div style={{marginBottom:'2%'}}>
                {
                    ...
                    <Card style={{boxShadow:'0px 0px 25px #333',marginTop:'2%',cursor:'pointer'}} onClick={(event) => this.callDetailed(event,article)}>
                        <Row>
                        <Card.Body>
                        <Col xs={12} lg={3} xl={3} md={3}  style={{}}>
                            <Card.Img src={this.checkSource(article.blocks.main.elements[0].assets)} height={'auto'} width={'auto'} 
                            style={{objectFit:"cover",padding:'1%',border:'solid 0.5px lightgray',float:'left',maxWidth:'120%'}} />
                            </Col>
