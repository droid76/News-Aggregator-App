...
class World extends Component 
{
    ...
    callGuardian()
    {
        fetch("https://csci571-node.azurewebsites.net/guardianWorld").then(res=> { 
        return res.json();
            }).then(news_articles=> this.setState({articles:news_articles}));
    }
    callNyt()
    {
        fetch("https://csci571-node.azurewebsites.net/nytWorld").then(res=> {
            return res.json();
                   ...
                   <Container fluid={true}>
                    <Card style={{boxShadow:'0px 0px 10px #333',marginTop:'2%',cursor:'pointer'}} onClick={(event) => this.callDetailed(event,article)}>
                        ...
                        <Col xs={12} lg={3} xl={3} md={3} sm={3} style={{}}>
                            ...
                            style={{objectFit:"cover",padding:'0.2%',border:'solid 0.5px lightgray',float:'left',maxWidth:'120%'}}/>
                            </Col>
